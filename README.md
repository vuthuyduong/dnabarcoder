...
The prediction component is designed to predict local and global similarity cut-offs for a reference dataset for sequence identification. For big datasets like the UNITE database, it is practical to compute local similarity cut-offs for species identification in genera. At higher taxonomic levels, to avoid memory constraints, it is practical to provide a maximum number of sequences selected for each clades for the prediction (see the examples below).
